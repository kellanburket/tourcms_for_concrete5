<?php 
	class VideoEmbed {
		...
		protected function youtube_embed() {
			return 'http' . ($this->secure ? "s" : "") . '://www.youtube.com/embed/' . $this->video_id . '?rel=0'  . ($this->autoplay ? "&autoplay=1" : "");
		}
		
		// Generate Vimeo embed code
		protected function vimeo_embed() {
			return 'http' . ($this->secure ? "s" : "") . '://player.vimeo.com/video/' . $this->video_id . ($this->autoplay ? "?autoplay=1" : "");
		}
	}
?>